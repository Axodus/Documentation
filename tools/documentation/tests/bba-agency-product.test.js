...
test('EPIC-006 closes while Product Vision 2.0 remains non-normative', async () => {
  const backlog = await read('.rag/bba-platform/EXECUTION-BACKLOG.yaml')
  ...
  const vision = await read('.rag/bba-platform/product/BBAPLT-GDE-101-PRODUCT-VISION-2-0-AGENCY-MODEL.md')
  const report = await read('.rag/bba-platform/product/BBAPLT-RPT-030-AGENCY-PRODUCT-VISION-CANONICAL-REVIEW.md')

  assert.match(backlog, /id: "EPIC-006"[\s\S]*status: "PASS_CLOSED"[\s\S]*status_reason: "AGENCY_PRODUCT_VISION_PASS"/)
  assert.match(backlog, /id: "SPRINT-006-06"[\s\S]*status: "PASS_CLOSED"[\s\S]*status_reason: "AGENCY_PRODUCT_CANONICAL_REVIEW_PASS"/)
  assert.match(backlog, /id: "REQ-006-06-006"[\s\S]*status: "DONE"/)
  assert.match(backlog, /id: "EPIC-005"[\s\S]*sprints: \[\]/)
  assert.match(report, /Review Result[\s\S]*PASS/)
  assert.match(report, /certifies the governance, consistency, traceability, and handoff/)
  assert.match(report, /does not certify `BBAPLT-GDE-101` as a normative/)
  assert.match(vision, /PROPOSED \/ NON_NORMATIVE_PENDING_REVIEW/)
  assert.doesNotMatch(backlog, /id: "EPIC-IMP-/)
  assert.doesNotMatch(graph, /to: "EPIC-IMP-/)
  assert.doesNotMatch(manifest, /EPIC-IMP-/)
})
