...
test('Logical Interfaces activates with six semantic REQs', async () => {
  ...
  const invariants = await read('.rag/bba-platform/architecture/BBAPLT-ARCH-010-LOGICAL-INTERFACE-INVARIANTS.md')
  assert.match(backlog, /id: "REQ-003-02-004"[\s\S]*status: "DONE"/)
  assert.match(invariants, /document_id: "BBAPLT-ARCH-010"/)
  for (const invariant of ['Identity Preservation', 'Tenant Boundary Preservation', 'Lineage Preservation', 'Semantic Integrity', 'Authority Preservation', 'Accountability Preservation', 'Ownership Preservation', 'Stewardship Preservation']) assert.match(invariants, new RegExp(invariant))
  for (const field of ['Motivation', 'Impact', 'Expected Violation', 'Architectural Consequence']) assert.match(invariants, new RegExp(field))
  assert.match(invariants, /does not redefine Domain concepts/)
  assert.match(invariants, /does not define[\s\S]*(API|protocol|runtime|deployment)/i)
  await access(resolve(root, '.rag/bba-platform/architecture/BBAPLT-ARCH-010-LOGICAL-INTERFACE-INVARIANTS.md'))
})
