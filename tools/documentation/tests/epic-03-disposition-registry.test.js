...
test('compact disposition registry represents all active governed dispositions after Batch 04', async () => {
  ...
  assert.equal(registry.entries.length, 563)
  assert.equal(new Set(registry.entries.map((entry) => entry.exception_id)).size, 563)
  assert.equal(new Set(registry.entries.map((entry) => entry.document_path)).size, 563)
...
test('REQ-04 compact disposition registry remains synchronized with baseline and exceptions', async () => {
  ...
  assert.equal(result.statistics.disposition_entries, 563)
  assert.equal(result.statistics.governed_dispositions, 563)
  assert.equal(result.statistics.undecided_dispositions, 0)
  assert.equal(result.statistics.temporary_extension_candidates, 386)
  assert.equal(result.statistics.migrate_dispositions, 177)
...
test('REQ-04 dry run applies no operational registry mutations', async () => {
  ...
  assert.equal(dryRun.statistics.would_apply_dispositions, 563)
  assert.equal(dryRun.statistics.governed_dispositions, 563)
...
test('REQ-05 application gate report records governed extension application only', async () => {
  const document = await loadDocument(resolve(root, '.rag/DOCUMENTATION-EXCEPTION-DISPOSITION-APPLICATION-GATE.md'), { root })
  const report = await readFile(resolve(root, '.rag/DOCUMENTATION-EXCEPTION-DISPOSITION-APPLICATION-GATE.md'), 'utf8')
