...
test('REQ-06 traceability-depth baseline is a connected canonical report', async () => {
  const document = await loadDocument(resolve(root, 'DOCUMENTATION-TRACEABILITY-DEPTH-BASELINE.md'), { root })
...
test('REQ-06 records the accepted default-overview-only cohort without mutations', async () => {
  const report = await read('DOCUMENTATION-TRACEABILITY-DEPTH-BASELINE.md')
