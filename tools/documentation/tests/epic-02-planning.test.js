...
test('REQ-02 approved contracts conform to Schema 1.0.0', async () => {
  const plan = await loadDocument(resolve(root, 'DOCUMENTATION-EPIC-02-PLAN.md'), { root })
...
test('initial scope registry covers the 16 detected public cores with stable allocation rules', async () => {
  const criteria = await readFile(resolve(root, 'DOCUMENTATION-MIGRATION-CRITERIA.md'), 'utf8')
...
test('debt classification precedence and flags are frozen', async () => {
  const classification = await readFile(resolve(root, 'DOCUMENTATION-DEBT-CLASSIFICATION.md'), 'utf8')
  const expected = [
    '`CONFLICTING`',
    '`DUPLICATED`',
    '`STALE`',
