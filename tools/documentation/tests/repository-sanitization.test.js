...
test('repository sanitization audit is a connected canonical report', async () => {
  const document = await loadDocument(resolve(root, 'DOCUMENTATION-REPOSITORY-SANITIZATION-AUDIT.md'), { root })
...
test('repository sanitization report records false-positive triage and closed gates', async () => {
  const report = await read('DOCUMENTATION-REPOSITORY-SANITIZATION-AUDIT.md')
