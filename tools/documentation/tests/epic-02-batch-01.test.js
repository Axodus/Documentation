...
test('Batch 01 evidence and final IDs cover the exact migration set', async () => {
  const evidence = await readFile(resolve(root, '.rag/DOCUMENTATION-MIGRATION-BATCH-01-EVIDENCE.md'), 'utf8')
  const ids = await readFile(resolve(root, '.rag/DOCUMENTATION-BATCH-01-ID-RESERVATION-RECORD.md'), 'utf8')
  for (const [path, id] of migrated) {
