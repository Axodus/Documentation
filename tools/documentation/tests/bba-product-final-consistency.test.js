...
test('Product final consistency review activates with eight audit REQs', async () => {
  ...
  assert.match(report, /PASS — Product Layer is consistent, complete, and certified for Architecture Layer entry/)
  ...
  assert.match(backlog, /id: "REQ-001-03-007"[\s\S]*status: "DONE"/)
  assert.match(report, /## Product Readiness Assessment/)
  assert.match(report, /classifies the Product Layer as `COMPLETE`/)
  assert.match(report, /CRITICAL \| 0/)
  assert.match(backlog, /id: "REQ-001-03-008"[\s\S]*status: "DONE"/)
  assert.match(report, /## Final Certification Decision/)
  assert.match(report, /EPIC-001` is `PASS_CLOSED` with reason `PRODUCT_LAYER_CERTIFIED`/)
  assert.match(report, /EPIC-003` remains `PLANNED`/)
