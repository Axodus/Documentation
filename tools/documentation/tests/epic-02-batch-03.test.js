...
test('Batch 03 evidence records IDs, freeze and zero substitutions', async () => {
  const evidence = await read('DOCUMENTATION-MIGRATION-BATCH-03-EVIDENCE.md')
  const ids = await read('DOCUMENTATION-BATCH-03-ID-RESERVATION-RECORD.md')
  const freeze = await read('DOCUMENTATION-BATCH-03-FREEZE-RECORD.md')
  for (const [path, id] of migrated) {
