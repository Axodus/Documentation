...
test('REQ-04 governance artifacts conform to Schema 1.0.0', async () => {
  const artifacts = [
    ['DOCUMENTATION-BATCH-01-AUTHORITY-EVIDENCE.md', 'DOC-RPT-006', 'APPROVED'],
    ['DOCUMENTATION-BATCH-01-FREEZE-RECORD.md', 'DOC-RPT-005', 'APPROVED'],
    ['DOCUMENTATION-BATCH-01-EXECUTION-CONTRACT.md', 'DOC-SPEC-007', 'DRAFT'],
  ]
...
test('freeze proposal contains 20 primaries and four ordered alternates', async () => {
  const proposal = await load('DOCUMENTATION-BATCH-01-CANDIDATE-FREEZE-PROPOSAL.md')
...
test('freeze record hashes the exact ordered candidate sets', async () => {
  ...
  const record = await load('DOCUMENTATION-BATCH-01-FREEZE-RECORD.md')
...
test('authority defaults and execution boundary are explicit', async () => {
  const evidence = await load('DOCUMENTATION-BATCH-01-AUTHORITY-EVIDENCE.md')
  const contract = await load('DOCUMENTATION-BATCH-01-EXECUTION-CONTRACT.md')
