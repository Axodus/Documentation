...
const artifacts = [
  ['DOCUMENTATION-EXCEPTION-DISPOSITION-INVENTORY.md', 'DOC-REF-010'],
  ['DOCUMENTATION-EXCEPTION-DISPOSITION-SUMMARY.md', 'DOC-RPT-037'],
  ['DOCUMENTATION-EXCEPTION-EXPIRY-RISK-REPORT.md', 'DOC-RPT-038'],
  ['DOCUMENTATION-EXCEPTION-AUTHORITY-ROUTING.md', 'DOC-RPT-039'],
  ['DOCUMENTATION-EXCEPTION-SECURITY-ROUTING.md', 'DOC-RPT-040'],
  ['DOCUMENTATION-EPIC-03-PROGRESS-REPORT.md', 'DOC-RPT-041'],
]
...
async function inventoryRows() {
  return (await read('DOCUMENTATION-EXCEPTION-DISPOSITION-INVENTORY.md'))
    .split('\n')
...
test('baseline and exception registries remain synchronized and unchanged in population', async () => {
...
test('renewal forecast applies P0 90-day cap without mutating exceptions', async () => {
  const report = await read('DOCUMENTATION-EXCEPTION-EXPIRY-RISK-REPORT.md')
...
test('routing summaries remain category-level and redacted', async () => {
  const authority = await read('DOCUMENTATION-EXCEPTION-AUTHORITY-ROUTING.md')
  const security = await read('DOCUMENTATION-EXCEPTION-SECURITY-ROUTING.md')
