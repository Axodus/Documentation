...
const artifacts = [
  ['.rag/DOCUMENTATION-LEGACY-DEBT-REPORT.md', 'DOC-RPT-003'],
  ['.rag/DOCUMENTATION-LEGACY-PRIORITIZATION.md', 'DOC-ROAD-002'],
  ['.rag/DOCUMENTATION-CORE-ADOPTION-MATRIX.md', 'DOC-REF-005'],
  ['.rag/DOCUMENTATION-LEGACY-CLASSIFICATION-EVIDENCE.md', 'DOC-RPT-004'],
  ['.rag/DOCUMENTATION-BATCH-01-CANDIDATE-FREEZE-PROPOSAL.md', 'DOC-ROAD-003'],
]
...
test('classification partition covers every legacy path exactly once', async () => {
  ...
  assert.equal(needsReview.length, 504)
  assert.equal(exceptionRequired.length + needsReview.length, 563)
...
test('domain distribution matches the accepted inventory', async () => {
  ...
  assert.equal(count('docs/'), 177)
  assert.equal(count('.knowledge/'), 285)
  assert.equal(count('.instructions/'), 59)
  assert.equal(563 - count('docs/') - count('.knowledge/') - count('.instructions/'), 42)
})

test('Batch 01 proposal retains 20 primaries and four ordered alternates without final IDs', async () => {
  const proposal = await readFile(resolve(root, '.rag/DOCUMENTATION-BATCH-01-CANDIDATE-FREEZE-PROPOSAL.md'), 'utf8')
...
test('core adoption matrix covers all 16 public scopes without migration or adoption states', async () => {
  const matrix = await readFile(resolve(root, '.rag/DOCUMENTATION-CORE-ADOPTION-MATRIX.md'), 'utf8')
