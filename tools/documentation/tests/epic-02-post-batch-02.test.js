...
test('REQ-08 artifacts are canonical with monotonic non-colliding IDs', async () => {
  const artifacts = [
    ['.rag/DOCUMENTATION-BATCH-02-ACCEPTANCE-REPORT.md', 'DOC-RPT-022'],
    ['.rag/DOCUMENTATION-BATCH-02-POST-MIGRATION-AUDIT.md', 'DOC-RPT-023'],
    ['.rag/DOCUMENTATION-TRACEABILITY-EXPANSION.md', 'DOC-SPEC-006'],
    ['.rag/DOCUMENTATION-GRAPH-ENRICHMENT-REPORT.md', 'DOC-RPT-024'],
    ['.rag/DOCUMENTATION-BATCH-03-PLANNING.md', 'DOC-ROAD-006'],
    ['.rag/DOCUMENTATION-BATCH-03-CANDIDATE-FREEZE-PROPOSAL.md', 'DOC-ROAD-007'],
    ['.rag/DOCUMENTATION-BATCH-03-AUTHORITY-GAP-REPORT.md', 'DOC-RPT-025'],
    ['.rag/DOCUMENTATION-BATCH-03-READINESS-FORECAST.md', 'DOC-RPT-026'],
  ]
...
test('Batch 02 acceptance and audit are explicit', async () => {
  assert.match(await read('.rag/DOCUMENTATION-BATCH-02-ACCEPTANCE-REPORT.md'), /Batch 02 Acceptance: ACCEPTED/)
  const audit = await read('.rag/DOCUMENTATION-BATCH-02-POST-MIGRATION-AUDIT.md')
...
test('traceability reports record current graph metrics without unresolved edges', async () => {
  const specification = await read('.rag/DOCUMENTATION-TRACEABILITY-EXPANSION.md')
...
test('Batch 03 proposal has 20 primaries, six alternates and no final IDs', async () => {
  const proposal = await read('.rag/DOCUMENTATION-BATCH-03-CANDIDATE-FREEZE-PROPOSAL.md')
