...
2. Root public inventory includes additional canonical and legacy Markdown
   outside `docs/`.
3. `.rag/` contains governance inputs plus generated governance
   outputs.
...
`.rag/`, but its README describes mixed planning, draft, and
...
- public generator outputs under root JSON and `.rag/`;
...
- `Documents/` and `.rag/` are distinct paths, but similar enough to
  confuse readers;
...
| `RISK-EP4-003` | `.rag/` | Authored and generated governance artifacts share one namespace | `MEDIUM` | no | explicit generated-path set and governance awareness | keep namespace; no rename |
| `RISK-EP4-004` | `Documents/` | Namespace visually collides with `.rag/` and over-broad rename target could distort planning semantics | `HIGH` | yes for execution freeze | README semantics, governed exceptions, generated inventory references | keep planning-only |
...
- Keep `.rag/` as the governance namespace and avoid a second
